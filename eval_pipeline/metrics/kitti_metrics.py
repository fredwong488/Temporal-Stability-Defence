"""
metrics/kitti_metrics.py
------------------------
KITTI-specific evaluation metrics matching the KITTI devkit (evaluate_object.cpp).

3D IoU      : see common.py (compute_iou_3d) — used here via _match_frame.
...
These metrics are not applicable to NuScenes, which uses a different class set,
no Easy/Moderate/Hard difficulty, and lacks bbox_2d pixel-height annotations.
"""
...
from ..types import FrameResult, ObjectLabel
from .common import _match_frame
...
def _match_frame_kitti(
    ...
    """Wrapper around _match_frame that injects the KITTI difficulty filter."""
    return _match_frame(
        predictions, labels, class_name, iou_threshold,
        score_threshold=score_threshold,
        difficulty=difficulty,
        _label_passes_difficulty_fn=_label_passes_difficulty,
    )
...
def compute_map(
    ...
    difficulties
        ...
        An "all" key (no difficulty filter) is always included.
...
def compute_pr_curve(
    ...
    monotone envelope applied, so the curve is non-increasing in precision as
    recall increases.
    ...
    dict with keys:
        "recall"    : list[float]
        "precision" : list[float] — monotone envelope
        "ap"        : float
        "n_gt"      : int
    """
    all_tp_scores: list[float] = []
