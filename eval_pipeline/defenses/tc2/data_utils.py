...
# Vendored from https://github.com/merlresearch/MotionNet/blob/main/data/data_utils.py — unchanged.
...
def gen_2d_grid_gt(
    ...
    for i in range(num_instances):
        ...
        if h_flip:
            ...
            tmp_box = Box(
                center=instance_box_data[:3], size=instance_box_data[3:6], orientation=Quaternion(instance_box_data[6:])
            )
...
def voxelize_occupy(pts, voxel_size, extents=None, return_indices=False):
    ...
    leaf_layout[voxel_indices[:, 0], voxel_indices[:, 1], voxel_indices[:, 2]] = VOXEL_FILLED
...
def voxelize_point_pillar(
    pts, grid_size, extents=None, num_points=100, num_pillars=2500, seed=None, is_padded_pillar=False
):
    """
...
def compute_ratio_cat_and_motion(
    ...
    for s_dir in scene_dirs:
        ...
        sample_files = [
            os.path.join(sample_dir, f) for f in os.listdir(sample_dir) if os.path.isfile(os.path.join(sample_dir, f))
        ]
    ...
    for idx in range(num_sample_seqs):
        ...
        all_disp_field_gt, all_valid_pixel_maps, non_empty_map, pixel_cat_map_gt = gen_2d_grid_gt(
            sample_file, grid_size=voxel_size[0:2], reordered=True, extents=area_extents, frame_skip=frame_skip
        )
...
def compute_speed_level(all_disp_field_gt, total_future_sweeps=20, frame_skip=3):
    ...
    speed_level = np.zeros(
        (all_disp_field_gt.shape[1], all_disp_field_gt.shape[2], speed_intervals.shape[0]), dtype=np.float32
    )
