"""
defenses/radial_jitter_bev.py
-----------------------------
BEV variant of the Radial Jitter Defense — identical to radial_jitter.py
except that DBSCAN clustering (current frame and all past sweeps) operates on
the birds-eye-view XY projection.  All subsequent computations (centroid
radial distances, ICP for σ_point) use the original 3-D XYZ coordinates.
...
class RadialJitterBEVDefense(BaseDefense):
    ...
    def detect(self, frame: Frame, history: FrameHistory) -> DetectionResult:
        ...
        # DBSCAN each past compensated sweep on BEV (XY only); 3D points kept.
